...
// sendDrainWhenRequested sends DRAIN independently of the receive loop. The
// receive loop can block indefinitely on an idle stream, but shutdown must
// stop the proxy server from assigning new requests before local endpoints
// begin terminating.
