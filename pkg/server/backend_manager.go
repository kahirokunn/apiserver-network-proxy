...
// one non-draining backend. Draining connections remain in storage so existing
// tunnels can finish, but they must not make the server ready or receive new
// dials.
...
func (s *DefaultBackendStorage) GetRandomBackend() (*Backend, error) {
	...
	for i := 0; i < len(s.agentIDs); i++ {
		...
		// Prefer the first connection to an agent, because the agent will
		// normally close later duplicates. A draining first connection must
		// not hide a usable replacement connection, however.
		for _, backend := range s.backends[agentID] {
			if !backend.IsDraining() {
